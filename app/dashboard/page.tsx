            ...
            {stats.map((s, i) => (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div
                    className="font-display text-[30px] sm:text-[38px] font-black leading-none"
                    style={{
                  ...
                  <div className="font-mono text-[10px] font-bold uppercase tracking-[1.5px] text-ash mt-1.5">
                    {s.label}
                  </div>
...
function Card({ v }: { v: (typeof verticals)[number] }) {
  const inner = (
    <>
      <div className="mb-4 flex items-start justify-between">
        {/* Icon container */}
        <div
          className="flex h-12 w-12 items-center justify-center rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(0,255,136,0.25)]"
          style={{
            background: v.live
        ...
        >
          <v.icon className={`h-5 w-5 transition-colors duration-300 ${v.live ? "text-neon-green" : "text-muted"}`} strokeWidth={2.2} />
        </div>
        ...
        {v.live ? (
          ...
          >
            <span className="h-1.5 w-1.5 rounded-full bg-pit pulse-ring" />
            Live
          </span>
        ) : (
      ...
      <h3 className={`mb-1 text-lg font-bold transition-colors duration-300 ${v.live ? "text-white group-hover:text-neon-green" : "text-ash/70"}`}>
        {v.name}
      </h3>
  ...
  const cardClasses = `glass-panel group flex h-full flex-col p-6 transition-all duration-300 relative overflow-hidden ${
    v.live
      ? "hover:border-neon-green/40 hover:shadow-[0_0_32px_rgba(0,255,136,0.12)] hover:bg-white/[0.06]"
      : ""
  }`;
